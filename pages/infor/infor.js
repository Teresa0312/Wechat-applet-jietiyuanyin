...
Page({
  ...
  if(options.type=='choice'){
    ...
    wx.request({
      ...
      data: {subid:options.id,cid:"0",ctype:1},
      ...
      success: function (res) {
        ...
        if(res.data.data.first === undefined || res.data.data.first.length == 0){
            that.setData({
              pre:"无",
              preIndex:""
            })
          ...
          if(res.data.data.next === undefined || res.data.data.next.length == 0){
            that.setData({
              next:"无",
              nextIndex:""
            })
    ...
    wx.request({
      ...
      data: {subid:options.id,sid:"0",ctype:1},
      method: 'POST', // OPTIONS, GET, HEAD, POST, PUT, DELETE, TRACE, CONNECT
      header: {
        'content-type': 'application/x-www-form-urlencoded'
      },// 设置请求的 header
  ...
  showQue:function(event){
    ...
    if(index&&that.data.type==1){
      wx.request({
        url: 'http://120.24.15.224:8089/index.php/Admin/Api/showChoices',
        data: {subid:that.data.sid,cid:index,ctype:1},
        method: 'POST', // OPTIONS, GET, HEAD, POST, PUT, DELETE, TRACE, CONNECT
        header: {
          'content-type': 'application/x-www-form-urlencoded'
        },// 设置请求的 header
      ...
      wx.request({
        url: 'http://120.24.15.224:8089/index.php/Admin/Api/showShorts',
        data: {subid:that.data.sid,sid:index,ctype:1},
        method: 'POST', // OPTIONS, GET, HEAD, POST, PUT, DELETE, TRACE, CONNECT
        header: {
          'content-type': 'application/x-www-form-urlencoded'
        },// 设置请求的 header
  ...
  checkAns:function(event){
    ...
    if(that.data.flag=="0"){
      ...
      wx.request({
        ...
        data: {cid:qid,answer:ans,ctype:1},
        method: 'POST', // OPTIONS, GET, HEAD, POST, PUT, DELETE, TRACE, CONNECT
        header: {
          'content-type': 'application/x-www-form-urlencoded'
        },// 设置请求的 header
  ...
  addMistake:function(id){
    var that=this
    var qid=id
    var tid=that.data.sid
    wx.request({
        url: 'http://120.24.15.224:8089/index.php/Admin/Api/addMistakes',
        data: {module:1,type:tid,qid:qid,uid:that.data.userInfo.id},
        method: 'POST', // OPTIONS, GET, HEAD, POST, PUT, DELETE, TRACE, CONNECT
        header: {
          'content-type': 'application/x-www-form-urlencoded'
        },// 设置请求的 header
